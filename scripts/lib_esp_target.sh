...
esp_ensure_mounted() {
    ...
    mp="$(mktemp -d)"
    # Read-only for the probe: callers mount EVERY ESP-typed partition just to
    # test whether rEFInd is on it, as root, including whatever removable media
    # is attached. Only the ESP actually chosen as the target gets remounted
    # writable, via esp_make_writable below.
