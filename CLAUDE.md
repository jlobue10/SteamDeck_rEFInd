...
- `Windows/GUI/install_rEFInd.ps1`, `install_config_from_GUI.ps1`, `rEFInd_bg_randomizer.ps1`, and `uninstall_rEFInd.ps1` are kept identical to the sibling rEFInd_GUI repo's `windows/` versions except `rEFInd_GUI` → `SteamDeck_rEFInd` and `"rEFInd GUI"` → `"SteamDeck rEFInd GUI"` — port upstream changes by copying + renaming. As of the 2.3.1 parity port the installer writes a dedicated `rEFInd` **Boot#### EFI variable directly** (`SetFirmwareEnvironmentVariableW`; bcdedit cannot create firmware entries on current Windows 11, and repointing `{bootmgr}` drags Windows' optional-data blob into the entry — rEFInd_GUI issue #23), restores a `{bootmgr}` repointed by older versions, never touches entries carrying the `"WINDOWS"` blob (hex `57494e444f5753`), and verifies via NVRAM read-back; the config installer and randomizer target the ESP the firmware rEFInd entry actually points at (`bcdedit /enum firmware` + `QueryDosDevice`) before falling back to a filesystem scan. `uninstall_rEFInd.ps1` reverses everything and is run elevated by the Inno uninstaller (`[UninstallRun]`+`Verb: runas` after a yes/no prompt; `[UninstallDelete]` scrubs the per-user app dir). `scripts/uninstall_rEFInd.sh` is the Deck-side counterpart (disables `bootnext-refind` FIRST — it recreates the boot entry every boot — then entries/Windows re-activation/ESP files/refind package; `--keep-esp-files`, `--remove-app`); keep the pair in behavioral parity.
