...
- **`OSDetector`** (`osdetect.*`): `osdetect_common.cpp` scans the ESP's `EFI/` vendor dirs for loaders (SteamOS `steamcl.efi`, Windows `bootmgfw.efi`, shim/GRUB/systemd-boot) and assembles cross-volume entries (SYSTEM labels, Batocera/Ventoy, removable ESPs by partition GUID). `osdetect_linux.cpp` enumerates via `lsblk -J` and finds the ESP — **`/esp` first** on the Deck. `osdetect_win.cpp` uses PowerShell + `mountvol`. The removable-ESP-by-partition-GUID path **fixes the old Windows-on-SD-card `volume` bug** that the README documents. As of the rEFInd_GUI 2.0.4 parity port, `detect()` iterates **every** ESP (not just one) and tags each entry with its ESP's partition GUID (`espVolumeId`), so generated stanzas boot regardless of which ESP rEFInd launched from; the four `osdetect` files are kept **byte-identical to the sibling rEFInd_GUI repo's** except for the Deck's `/esp` mountpoint in `isRunningSystemEsp()` (`osdetect_common.cpp`) — when pulling upstream detection fixes, copy the files and re-apply that one hunk. (`isLegionGo`/`isLegionGo2` come along for the ride; the Deck fork's `mainwindow.cpp` hardcodes `resolution 3` and never calls them.)
...
- `Windows/GUI/install_rEFInd.ps1` and `install_config_from_GUI.ps1` are kept identical to the sibling rEFInd_GUI repo's `windows/` versions except `rEFInd_GUI` → `SteamDeck_rEFInd` in `%LOCALAPPDATA%` paths — port upstream changes by copying + renaming. The installer runs every `bcdedit` call through the exit-code-checked `Invoke-Bcdedit` wrapper (native exe failures don't trip `$ErrorActionPreference='Stop'`), snapshots `{bootmgr}`/`{fwbootmgr}` before modifying, puts Windows Boot Manager first in the firmware boot order, and ends with a read-back SUCCESS/WARNING/FAILED summary.
