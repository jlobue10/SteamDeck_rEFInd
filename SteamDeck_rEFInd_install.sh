...
# TouchI2cDxe touchscreen UEFI driver (jlobue10/TouchI2cDxe): the OLED Deck's
# (Galileo) FocalTech touch panel is HID-over-I2C, which a USB driver
# structurally cannot see; this driver produces AbsolutePointer so the rEFInd
# menu is touch-usable, including rotating the portrait touch matrix onto
# rEFInd's landscape mode. LCD Decks (Jupiter) have no supported profile and
# skip the download. Like the controller driver, download failure is non-fatal.
