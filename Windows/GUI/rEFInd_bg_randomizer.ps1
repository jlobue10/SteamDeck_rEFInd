...
function Get-RefindNvramEsp($esps) {
    ...
    # Prefer entries whose loader path is \EFI\refind\refind*.efi; then fall back
    # to any firmware entry the Linux refind-install labelled exactly "rEFInd".
    # bcdedit can render an efibootmgr-created entry's path in a form the strict
    # match misses (or as "device unknown"); without this fallback the scan below
    # would prefer the Windows ESP and shadow the live rEFInd on the Linux ESP.
