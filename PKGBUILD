...
package() {
	...
	install -Dm644 "${srcdir}/$_pkgname/SteamDeck_rEFInd.png" "${pkgdir}/usr/share/icons/hicolor/256x256/apps/SteamDeck_rEFInd.png"
 	install -Dm777 "${srcdir}/$_pkgname/scripts/rEFInd_bg_randomizer.sh" "${pkgdir}/usr/bin/install_config_from_GUI.sh"
 	install -Dm777 "${srcdir}/$_pkgname/scripts/rEFInd_bg_randomizer.sh" "${pkgdir}/usr/bin/pacman_install.sh"
  	install -Dm777 "${srcdir}/$_pkgname/scripts/rEFInd_bg_randomizer.sh" "${pkgdir}/usr/bin/sourceforge_install.sh"
