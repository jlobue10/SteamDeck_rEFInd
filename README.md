...
- **Browse dialog shows no PNG previews / "view as icons" option (KDE)** — not an app bug: the picker requests the desktop's native file dialog, which only shows thumbnails and view options when the Qt **platform-integration** plugin matching the GUI's Qt version is installed. SteamOS ships it, so the Deck is usually fine; on another KDE setup where previews are missing, install `plasma-integration` for the right Qt version (`ldd $(command -v SteamDeck_rEFInd)` — look for `libQt6Widgets` vs `libQt5Widgets`) and relaunch. Without it Qt falls back to a bare dialog with no previews.
